...
ACC.navigation = {
    ...
    generalNavigation: function() {
        var MAINE_NODE_NAME = 'navigation-node';
        var ACTIVATED_MENU_CLASS_NAME = 'navigation-active';
        var MOBILE_NAV_CONTAINER = 'mobile-nav';
        var MOBILE_NAV_CONTENT = 'mobile-nav--navigation';
        var HAMBURGER_BUTTON = 'mobile-hamberger';
        var CLOSE_BUTTON = 'mobile-nav--button';
        ...
        var desctopMenu = function() {
            var navNodes = Array.from(document.querySelectorAll('.' + MAINE_NODE_NAME));
            if(navNodes.length > 0) {
                navNodes.forEach(li => {
                    li.addEventListener('mouseover', () => {
                        if(!li.classList.contains(ACTIVATED_MENU_CLASS_NAME)) {
                            li.classList.add(ACTIVATED_MENU_CLASS_NAME)
                        }
                    });
                    li.addEventListener('mouseout', () => {
                        if(li.classList.contains(ACTIVATED_MENU_CLASS_NAME)) {
                            li.classList.remove(ACTIVATED_MENU_CLASS_NAME)
                        }
                    });
                });
            } else {
        ...
        var mobileMenu = function() {
            var hamburger = document.getElementsByClassName(HAMBURGER_BUTTON)[0];
            var navContainer = document.getElementsByClassName(MOBILE_NAV_CONTAINER)[0];
            var navContent = document.getElementsByClassName(MOBILE_NAV_CONTENT)[0];
            var closeBtn = document.getElementsByClassName(CLOSE_BUTTON)[0]
            if(!hamburger) {
                throw new Error('Hamburger button doesn`t exist.');
            } else {
